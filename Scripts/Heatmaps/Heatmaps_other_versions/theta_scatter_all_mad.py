...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr  3 11:18:09 2023
...
for stain in stains:
    ...
    f6.subplots_adjust(hspace=0.125)
    ...
    for im, ax, ax1, ax2, ax3, ax4, ax5 in zip(images, axs1.flatten(), axs2.flatten(), axs3.flatten(), axs4.flatten(), axs5.flatten(), axs6.flatten()):
        ...
        if well=="C05":
            ...
            err_bars_theta=np.array(theta_groups.transform(MAD).groupby(feat_filt['theta_bin_cv']).mean())[:-1]
            ax4.errorbar(theta_medians.index, theta_medians.values, yerr=err_bars_theta, capsize=10, color="w")
            ...
            err_bars_phi=np.array(phi_groups.transform(MAD).groupby(feat_filt['phi_bin_cv']).mean())[:-1]
            ax5.errorbar(phi_medians.index, phi_medians.values, yerr=err_bars_phi, capsize=10, color="w")
            ...
            err_bars_theta=np.array(theta_groups.transform(MAD).groupby(feat_filt['theta_bin_cv']).mean())
            ax4.errorbar(theta_medians.index, theta_medians.values, yerr=err_bars_theta, capsize=10, color="w")
            ax4.set_xticks(np.unique(feat_filt.theta_bin_cv), labels=np.around(theta_labs, 2), rotation=90)
            ax4.set_xlabel("Theta_bin")
            ax4.set_ylabel(stain+" median")
            ...
            err_bars_phi=np.array(phi_groups.transform(MAD).groupby(feat_filt['phi_bin_cv']).mean())
            ax5.errorbar(phi_medians.index, phi_medians.values, yerr=err_bars_phi, capsize=10, color="w")
            ax5.set_xticks(np.unique(feat_filt.phi_bin_abs), labels=np.around(phi_labs_abs, 2), rotation=90)
            ax5.set_xlabel("Phi_bin")
            ax5.set_ylabel(stain+" median")
    ...
    f5.savefig(path_out_im+stain_clean+"_median_ov_theta_mad.png", bbox_inches='tight', dpi=300)

    f6.savefig(path_out_im+stain_clean+"_median_ov_phi_mad.png", bbox_inches='tight', dpi=300)
