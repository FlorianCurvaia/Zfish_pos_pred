...
# -*- coding: utf-8 -*-
"""
Created on Thu Mar 23 12:26:10 2023
...
for s in range(l):
    ...
    feat_filt_99=feat_filt.loc[feat_filt[stain]<vmax]
    #ax.set_title(stain_clean)
    g = sns.jointplot(data=feat_filt_99, x="cur_phi", y=stain, hue="hpf", kind="kde", palette="viridis", joint_kws={'alpha': 0.5})
    g.ax_joint.set_xticks(theta_bins, theta_labs)
    #g.ax_joint.set_yticks(phi_bins, phi_labs)
    g.ax_joint.set_ylabel(stain)
    g.ax_joint.set_xlabel("theta")
    plt.savefig(path_out_im+stain_clean+"_scatter_kde.png", bbox_inches='tight', dpi=300)
    plt.close()
