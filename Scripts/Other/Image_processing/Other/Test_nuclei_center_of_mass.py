...
results=[]
for nuc in nuc_ids:
    results.append(get_cm(seg_nuc, nuc))
...
np.mean(np.nonzero(seg_nuc==1),1)
print("np.where --- %s seconds ---\n" % (time.time() - start_time_1))

start_time_2=time.time()
tu=np.nonzero(seg_nuc==1)
to=[]
for i in range(3):
    to.append(sum(tu[i])/len(tu[i]))
#print(to)
print("np.nonzero --- %s seconds ---\n" % (time.time() - start_time_2))