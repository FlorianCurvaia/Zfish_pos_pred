...
# -*- coding: utf-8 -*-
"""
Created on Tue May  9 14:04:44 2023
...
path_out_im="/Users/floriancurvaia/Desktop/Uni/ETH/Labos/Pelkmans/work/fcurvaia/pre_aligned/Images/DV_axis/"
...
for idx in range(len(images)):
    ...
    feat_filt_marg=feat_filt.loc[feat_filt.rel_theta>0.8]
    ...
    means_ov_phi=feat_filt_marg.groupby(['phi_bin_DV'])["betaCatenin_nuc"].mean()
    means_ov_phi_2=feat_filt.groupby(['phi_bin_DV'])["pSmad1/5_nc_ratio"].mean()
    means_ov_phi_3=means_ov_phi+means_ov_phi_2
    new_phi_min=phi_bins[means_ov_phi_3.tolist().index(np.nanmin(means_ov_phi_3))]
...
print(results)
#plt.scatter(results.hpf, results.min_2_diff.abs())
plt.hist(results.loc[results.hpf>5]["min_2_diff"].abs(), bins=36)
#results.min_2_diff.plot.hist(bins=(int(math.floor(results.min_2_diff.max()-results.min_2_diff.min())/12)))
plt.show()
