...
def sphereFit(coords):
    ...
    return radius, C[0], C[1], C[2] #C[0]/0.65, C[1]/0.65
...
for im in args.images:
    ...
    #np.save(path_out+"mid_sphere_"+im, mid_sphere_ed)
    ...
    #fn_dist="/Users/floriancurvaia/Desktop/Uni/ETH/Labos/Pelkmans/work/fcurvaia/features/C05_px+0198_py+1683.csv"
    #fn_dist="/Users/floriancurvaia/Desktop/Uni/ETH/Labos/Pelkmans/work/fcurvaia/distances/C05_px+0198_py+1683_w_dist_sph_simp.csv" #B08_px+1076_py-0189_w_dist_sph_simp.csv"
    ...
    #features=features.drop(features[features.MapK_2_Mean==0].index)
    ...
    """
    feat_filt["pSmad1/5_nc_ratio"]=(feat_nuc["pSmad1/5_4_Mean"].to_numpy()/feat_cyto["pSmad1/5_4_Mean"].to_numpy())
    
    feat_filt.replace([np.inf, -np.inf], np.nan, inplace=True)
    #feat_filt["pSmad1/5_nc_ratio"] = feat_filt["pSmad1/5_nc_ratio"].replace(np.nan, 0)
    
    feat_filt["pSmad1/5_nc_ratio"].fillna(feat_nuc["pSmad1/5_4_Mean"], inplace=True)
    
    feat_filt["betaCatenin_nc_ratio"]=(feat_nuc["betaCatenin_0_Mean"].to_numpy()/feat_cyto["betaCatenin_0_Mean"].to_numpy())
    
    feat_filt.replace([np.inf, -np.inf], np.nan, inplace=True)
    
    feat_filt["betaCatenin_nc_ratio"].fillna(feat_nuc["betaCatenin_0_Mean"], inplace=True)
    
    feat_filt["betaCatenin_nuc"]=feat_nuc["betaCatenin_0_Mean"]
    
    feat_filt["MapK_nc_ratio"]=(feat_nuc["MapK_2_Mean"].to_numpy()/feat_cyto["MapK_2_Mean"].to_numpy())
    
    feat_filt.replace([np.inf, -np.inf], np.nan, inplace=True)
    
    feat_filt["MapK_nc_ratio"].fillna(feat_nuc["MapK_2_Mean"], inplace=True)
    #feat_filt["betaCatenin_nc_ratio"] = feat_filt["betaCatenin_nc_ratio"].replace(np.nan, 0)
    """
    #"betaCatenin_0_Mean"
    ...
    #np.save(path_out+"out_sphere_"+im, out_sphere_ed)
