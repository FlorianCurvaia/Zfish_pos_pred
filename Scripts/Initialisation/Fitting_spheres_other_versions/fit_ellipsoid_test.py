...
# -*- coding: utf-8 -*-
"""
Created on Mon Feb 27 16:07:39 2023
...
points_out=np.zeros((out_elli.shape))
...
n = plt.imshow(mid_elli_ed[idx0], vmin=0, vmax=1, alpha=0.35)
p = plt.imshow(out_elli_ed[idx0], vmin=0, vmax=1, alpha=0.25)
k = plt.imshow(in_elli_ed[idx0], vmin=0, vmax=1, alpha=0.15)
...
def update(val):
    ...
    k.set_data(in_elli_ed[int(idx)])
    #o.set_data(points_out[int(idx)])
    n.set_data(mid_elli_ed[int(idx)])
    p.set_data(out_elli_ed[int(idx)])
