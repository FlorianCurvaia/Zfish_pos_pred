...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr 12 17:05:30 2023
...
path_in="/data/homes/fcurvaia/distances_new/"   #"/Users/floriancurvaia/Desktop/Uni/ETH/Labos/Pelkmans/work/fcurvaia/distances/" #"/data/homes/fcurvaia/distances/" 
path_out_im="/data/homes/fcurvaia/Images/Posterior/"   #"/Users/floriancurvaia/Desktop/Uni/ETH/Labos/Pelkmans/work/fcurvaia/Images/Posterior/" #"/data/homes/fcurvaia/Images/Pos_inf/"
...
path_out_im="/Users/floriancurvaia/Desktop/Uni/ETH/Labos/Pelkmans/Images/Posterior/"
#hpf=7
...
for s in range(len(stains)):
    ...
    for im, e in zip(files, range(0,len(files))):
        ...
        feat_filt['theta_bin_pos'] = pd.to_numeric(pd.cut(x = feat_filt['rel_theta'], bins = theta_bins, labels = labels, include_lowest = True, right=True))
...
#to_cov_theta={theta_bin:np.zeros((len(stains), len(files))) for theta_bin in range(1, int(min(all_t_max))+1)}
...
for embryo in files:
    ...
    P_x_phi=np.array((n_bins-1)*[1/(n_bins-1)])
    P_x_theta=np.histogram(feat_filt.rel_theta, bins=n_bins-1, range=(0,1))[0]/len(feat_filt)
    P_x_theta==np.array((n_bins-1)*[1/(n_bins-1)])
    ...
    max_theta=feat_filt.theta_bin_pos.max()
    ...
    fig, ax=plt.subplots()
    phi=ax.imshow(posterior_phi, aspect="auto", cmap="inferno")
    ...
    fig, ax=plt.subplots()
    theta=ax.imshow(posterior_theta, aspect="auto", cmap="inferno")
