...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr 12 09:47:25 2023
...
to_cov_phi={phi_bin:np.zeros((len(stains), len(files)*2)) for phi_bin in range(1, n_bins)}
to_cov_theta={theta_bin:np.zeros((len(stains), len(files)*2)) for theta_bin in range(1, min(all_t_max)+1)}
...
cov_phi={phi_bin:np.cov(mat) for phi_bin, mat in to_cov_phi.items()}
cov_theta={theta_bin:np.cov(mat) for theta_bin, mat in to_cov_theta.items()}
...
prob_time=time.time() - start_time_0
...
fig, ax=plt.subplots()
ax.imshow(posterior_phi, aspect="auto", cmap="turbo")
fig.savefig(path_out_im+"split_posterior_phi_"+embryo+"_"+str(n_bins)+".png", bbox_inches='tight', dpi=300)
#plt.close()
    

fig, ax=plt.subplots()
ax.imshow(posterior_theta, aspect="auto", cmap="turbo")
fig.savefig(path_out_im+"split_posterior_theta_"+embryo+"_"+str(n_bins)+".png", bbox_inches='tight', dpi=300)
