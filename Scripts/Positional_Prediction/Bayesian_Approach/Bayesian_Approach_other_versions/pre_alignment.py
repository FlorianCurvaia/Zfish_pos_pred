...
# -*- coding: utf-8 -*-
"""
Created on Thu May 11 10:14:11 2023
...
path_out=path_in.parent / (path_in.name+"_pre")
...
with h5py.File(fn) as f:
    for dset in h5_select(f, {'level': 1, 'cycle': ref_cycle}):
        cycle = dset.attrs['cycle']
        scale = dset.attrs['element_size_um']
        all_datasets_ref[dset.attrs["stain"]]=to_numpy(dset)
...
for cycle in cycles_to_pre_align:
    all_datasets={}
    with h5py.File(fn) as f:
        for dset in h5_select(f, {'level': 1, 'cycle': cycle}):
            all_datasets[dset.attrs["stain"]]=to_numpy(dset)

    
    DAPI_c_bool=all_datasets["DAPI"]>np.quantile(all_datasets["DAPI"], 0.99)
    ...
    for stain, array in all_datasets.items():
        if cycle>2:
            arr_=array[40:]
        else:
            arr_=array
        arr_=np.roll(arr_, x_shift, 1)
        arr_=np.roll(arr_, y_shift, 2)
        all_datasets[stain]=arr_
    ...
    with h5py.File(fn_out, "a") as f_out:
        with h5py.File(fn) as f_in:
            for stain, array in all_datasets.items():
                for dset in h5_select(f_in, {'stain': stain, 'level': 1, 'cycle': cycle}):
                    h5_write_channel(f_out, array, dset.name, copy_from=dset)
    
    del all_datasets
                
with h5py.File(fn_out, "a") as f_out:
    with h5py.File(fn) as f_in:
       for stain, array in all_datasets_ref.items():
           for dset in h5_select(f_in, {'stain': stain, 'level': 1, 'cycle': ref_cycle}):
               h5_write_channel(f_out, array, dset.name, copy_from=dset)
