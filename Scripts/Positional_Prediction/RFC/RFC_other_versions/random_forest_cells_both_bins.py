...
# -*- coding: utf-8 -*-
"""
Created on Mon May 22 15:35:30 2023
...
path_out_im="/data/homes/fcurvaia/Images/Random_Forest/both_bins/Cells_pred/" 
...
for h in range(2):
    ...
    for emb in np.unique(feat_filt.emb):
        ...
        #ax.set_title(im)
        fig.savefig(path_out_im+str(h)+"_"+emb+"_"+str(n_bins)+"_plot_random_forest.png", bbox_inches='tight', dpi=300)
        plt.close()
        ...
        """
        fig, ax=plt.subplots()
        sns.heatmap(conf_mat, cmap="inferno",  ax=ax) #annot=True,
        ...
        fig.savefig(path_out_im+str(h)+"_"+emb+"_"+str(n_bins)+"_conf_mat_random_forest.png", bbox_inches='tight', dpi=300)
        plt.close()
        
    mean_conf_mat=np.nanmean(np.array(all_conf_mat), axis=0)
    fig, ax=plt.subplots()
    sns.heatmap(mean_conf_mat, cmap="inferno",  ax=ax) #annot=True,
    #ax.set_title(im+" "+str(hpf)+" hpf")
    ax.set_yticks(np.array(list(range(0, n_bins)))+0.5, index, rotation=90)  #bins_coord_ticks
    ax.set_xticks(np.array(list(range(0, n_bins)))+0.5, index, rotation=90) #bins_coord_ticks
    ax.set_ylabel("True Label")
    ax.set_xlabel("Predicted Label")
    fig.savefig(path_out_im+str(h)+"_"+str(n_bins)+"_mean_conf_mat_random_forest.png", bbox_inches='tight', dpi=300)
    plt.close()
