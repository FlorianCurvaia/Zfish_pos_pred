...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar 28 09:07:49 2023
...
y_pred = cross_val_predict(model, x_all, y_all, cv=5) #model_results.predict(x_test)
...
for s in range(l):
    ...
    fig, ax = plt.subplots()
    plt.clf()
    ax_=plt.imshow(confusion_matrix(x_plot, y_plot, normalize="true"), cmap="turbo")
    fig.colorbar(ax_, ax=ax, label="N")
    plt.title(col)
    plt.savefig(path_out_im+im+"_"+col+"_random_forest_conf_mat.png", bbox_inches='tight', dpi=300)
    """
    disp = ConfusionMatrixDisplay.from_predictions(x_plot, y_plot, cmap=plt.cm.turbo, normalize="true")
    disp.figure_.savefig(path_out_im+im+"_"+col+"_random_forest_conf_mat.png", bbox_inches='tight', dpi=300)
