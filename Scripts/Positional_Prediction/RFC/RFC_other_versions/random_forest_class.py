...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr  4 16:46:52 2023
...
for f in fld.glob("*.csv"):
    ...
    feat_filt=pd.read_csv(fn, sep=",", index_col=False, usecols=["theta", "dist_out", "cur_phi", "phi_bin_cur"]+stains)
    feat_filt["emb"]=emb
...
for im, ax1, ax2, ax3, ax4 in zip(images, axs1.flatten(), axs2.flatten(), axs3.flatten(), axs4.flatten()):
    ...
    axs_sub=[ax1, ax2]
    axs_conf=[ax3, ax4]
    for s, sub_ax, ax_conf in zip(range(l), axs_sub, axs_conf):
        col=to_pred[s]
        ...
        sub_ax.plot(bins_tog[s], all_lengths)
        sub_ax.set_xlabel(col)
        sub_ax.set_ylabel("N bins")
        sub_ax.set_title(im)
        ...
        """
        sns.heatmap(conf_mat, cmap="turbo", annot=True, ax=ax_conf)
        ax_conf.set_title(im+" "+str(hpf)+" hpf")
        ax_conf.set_ylabel("True Label")
        ax_conf.set_xlabel("Predicted Label")
        
        
        
f1.savefig(path_out_im+"Theta_plot_random_forest.png", bbox_inches='tight', dpi=300)
f2.savefig(path_out_im+"Phi_plot_random_forest.png", bbox_inches='tight', dpi=300)
f3.savefig(path_out_im+"Theta_bins_random_forest_conf_mat.png", bbox_inches='tight', dpi=300)
f4.savefig(path_out_im+"Phi_bins_random_forest_conf_mat.png", bbox_inches='tight', dpi=300)
plt.close()
