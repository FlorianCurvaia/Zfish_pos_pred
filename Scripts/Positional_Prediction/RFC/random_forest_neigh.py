...
# -*- coding: utf-8 -*-
"""
Created on Fri Jun 23 15:09:22 2023
...
path_out_im="/data/homes/fcurvaia/Images/Random_Forest/both_bins/By_hpf/Neighbours/" 
...
for emb in np.unique(feat_filt.emb):
    ...
    train=feat_filt.loc[feat_filt.emb!=emb][stains_pred+to_pred]

    
    test=feat_filt.loc[feat_filt.emb==emb][stains_pred+to_pred]
    ...
    model = RandomForestClassifier(n_estimators = 200, random_state = 42, n_jobs=8)
    ...
    ax.set_yticks(np.array(list(range(0, n_bins))), index) #, rotation=90 #bins_coord_ticks
    ax.set_xticks(np.array(list(range(0, n_bins))), index, rotation=90) #bins_coord_ticks
