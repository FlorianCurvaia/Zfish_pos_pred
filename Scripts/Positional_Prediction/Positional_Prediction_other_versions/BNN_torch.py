...
# -*- coding: utf-8 -*-
"""
Created on Thu Jun 22 11:35:53 2023
...
model = nn.Sequential(
    bnn.BayesLinear(prior_mu=0, prior_sigma=1, in_features=3, out_features=1000),
    nn.ReLU(),
...
for step in range(1000):
    pre = model(torch.from_numpy(X_train.to_numpy()).float())
    mse = mse_loss(pre, torch.from_numpy(y_train.to_numpy()).float())
    kl = kl_loss(model)
...
models_result = np.array([model(torch.from_numpy(X_test.to_numpy()).float()).data.numpy() for k in range(1000)])
print(models_result.shape)
#models_result = models_result[:,:,0]    
#models_result = models_result.T

#mean_values = np.array([models_result[i].mean(axis=0) for i in range(len(models_result))])
#std_values = np.array([models_result[i].std(axis=0) for i in range(len(models_result))])
